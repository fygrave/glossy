...
function parseMessage(rawMessage, callback) {
   ...
   for (var i=0; i< rawMessage.length; i++) {
    ...
    var segments = null;
    if (rawMessage.match(/Imperva/)) {
	 segments = rawMessage.split(',');
    } else {
         segments = rawMessage.split(' ');
    }
    ...
    for (part in segments) {
        var kv  = segments[part].split('=');
        parsedMessage[kv[0]] = kv [ 1 ];
    }

    //TODO Could our detection between 3164/5424 be improved?
    if (segments[0].match(/Imperva/)) {
	parsedMessage.type='Imperva'
        segments.shift(); // Shift the prival off
        parsedMessage.src = parsedMessage.srcIP
	parsedMessage.dst = parsedMessage.dstIP
...
function decodePri(message)
{
    ...
    //FIXME These regexes are needlessly slow
    var privalMatch = message.match(/^.*<(\d+)>(\d)/);
